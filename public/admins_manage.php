...
<div class="container-fluid mypad">
 <form action='http://localhost/website/public/admin_new.php' method='post'>
...
if (mysqli_num_rows($result1) > 0) {
 ...
 <ul  class="list-group" >
 <li class="list-group-item list-group-item-warning"><a href="http://localhost/website/public/admins_manage.php?admin=<?php echo   mysqli_real_escape_string($conn,$row1["id"]);  ?> "><?php echo  mysqli_real_escape_string($conn,$row1["username"]);   ?></a></li>
  </ul>
<?php }}?>  
...
if (mysqli_num_rows($result) > 0) {
    while($row = mysqli_fetch_assoc($result)) {
    	?> 
<div class="container-fluid mypad">
 <form method='post'  action='http://localhost/website/public/admin_edit.php'>
  <div class="form-group">
    <label  >First name:</label>
...
if (mysqli_num_rows($result) > 0) {
    while($row = mysqli_fetch_assoc($result)) {
      ...
      <tr>
        ...
        <td><?php echo htmlentities($row["email"]);  ?></td>
        <td><?php echo htmlentities($row["date"]);  ?></td>
        <td><a type="button" class="btn btn-danger"    href="http://localhost/website/public/admin_delete.php?id=<?php echo   mysqli_real_escape_string($conn,$row["id"]);   ?> ">Delete</td>
      </tr> 
